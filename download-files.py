...
while n < number_sheets:
    ...
    while i < num_rows:
        ...
        url = str(row[0])   #读取列表第1个，转换数据类型（这里这一步非必要，已是string），正则表达式匹配字符串
        ...
        open(folder_path + filename + '_' + str(i+1) + '.zip', 'wb').write(file.content)  #打开一个文件对象写进url内容，文件不存在时会自动生成，即压缩包文件
        i = i + 1
    print(worksheets[n])
    n = n + 1
    i = 0
...
URL解码：https://www.urldecoder.io/python/
xlrd模块读取.xls文件：https://www.blog.pythonlibrary.org/2014/04/30/reading-excel-spreadsheets-with-python-and-xlrd/
创建文件夹：https://www.geeksforgeeks.org/create-a-directory-in-python/
'''
